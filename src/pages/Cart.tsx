...
import PizzaBasket from '../components/PizzaBasket/PizzaBasket'
import { ICartItem, IState } from '../@types/interface';
...
const Cart: React.FC = () => {
    ...
    const items: ICartItem[] = useSelector((state: IState) => state.cart.items)
    ...
    React.useEffect(() => {
        if(items.length !== 0) {
            ...
            for(let i = 0; i < items.length; i++) {
                ...
                intermediate_arr.push(
                    <PizzaBasket key={'piza_item' + item.id_unique} {...items[i]}/>
                )
            }

            setPizzas(intermediate_arr)
        }
    }, [items]);
                ...
                <div className="container">
                    <div className="content">
                        {items.length !== 0 ? (
                            ...
                            <>
                                <Link to="/" className='link_go_back'>
                                    <h2>
                                        Вы еще не заказали пиццу!
                                    </h2>
